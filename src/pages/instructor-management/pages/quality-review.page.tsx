...
import { Avatar, Button, Card, Modal, Row, Select, Table, Tag } from "antd";
...
const InstructorManagementPage: FC = () => {
  const [teamData, setTeamData] = useState<{
    loaded: boolean;
    data: InstructorManagementResponse["items"] | null;
  ...
  const getTeam = useCallback(() => {
    UserManagementApi.instructorManagement()
      .then(({ data }) => {
  ...
  const columns: ColumnsType<IInstructor> = useMemo(
    () => [
      {
        title: "Họ tên",
        dataIndex: "fullname",
      ...
      },
      {
        ...
        width: 200,
        render: (value) => {
          return <div>{value} số học viên</div>;
        },
      },
      {
        title: "Doanh thu",
    ...
    <div>
      ...
      <StyledTable>
        <Row align="middle" justify="space-between" className="mb-3">
          <Title level={3}>Danh sách giảng viên</Title>
        </Row>
        <Table
